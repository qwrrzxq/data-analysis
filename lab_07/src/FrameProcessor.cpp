...
void FrameProcessor::drawInfo(cv::Mat& frame, const KeyProcessor& keyProc, int fps) {
    ...
    auto putText = [&](const std::string& text) {
        cv::putText(frame, text, cv::Point(10, y), cv::FONT_HERSHEY_SIMPLEX,
                    scale, cv::Scalar(0, 0, 0), thickness + 2);
        cv::putText(frame, text, cv::Point(10, y), cv::FONT_HERSHEY_SIMPLEX,
    ...
    putText("Face detect: " + std::string(keyProc.shouldDetectFaces() ? "ON" : "OFF"));
    if (keyProc.shouldDetectFaces()) {
        putText("Delay sim: " + std::string(keyProc.shouldSimulateDelay() ? "500ms" : "OFF"));
    }
    ...
    y = frame.rows - 15;
    cv::putText(frame, "Keys: 0-7 modes | f face | +/- zoom | h/v flip | i info | r reset | q quit",
